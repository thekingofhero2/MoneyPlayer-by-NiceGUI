...
async def _generate_response_stream(prompt: str) -> AsyncGenerator[str, None]:
    """
    异步流式生成响应，逐块返回 LLM 生成的内容。
    """
    try:
        llm_provider = config.app.get("llm_provider", "openai")
        logger.info(f"llm provider: {llm_provider}")
        ...
        if llm_provider == "g4f":
            ...
            if not model_name:
                model_name = "gpt-3.5-turbo-16k-0613"
            # g4f 不支持真正的流式输出，直接返回完整内容
            content = g4f.ChatCompletion.create(
                model=model_name,
                messages=[{"role": "user", "content": prompt}],
            )
            yield _normalize_text_response(content, llm_provider)
        else:
            api_version = ""  # for azure
            if llm_provider == "moonshot":
                api_key = config.app.get("moonshot_api_key")
            ...
            if llm_provider not in ["pollinations", "ollama", "litellm"]:
                if not api_key:
                    raise ValueError(
                        f"{llm_provider}: api_key is not set, please set it in the config.toml file."
                    )
            ...
            if llm_provider == "qwen":
                    ...
                    for chunk in response:
                        if isinstance(chunk, GenerationResponse):
                            if chunk.status_code != 200:
                                raise Exception(
                                    f'[{llm_provider}] returned an error response: "{chunk}"'
                                )
                            text = _extract_qwen_generation_text(chunk)
                            if text:
                                yield text
                        else:
                            raise Exception(
                                f'[{llm_provider}] returned an invalid response: "{chunk}"'
                            )
                else:
                    raise Exception(f"[{llm_provider}] returned an empty response")
                ...
                result = response.json()
                logger.info(result)
                yield _normalize_text_response(result["result"]["response"], llm_provider)

            elif llm_provider == "ernie":
                # ERNIE 不支持流式，直接返回完整响应
                response = requests.post(
                    "https://aip.baidubce.com/oauth/2.0/token", 
                    params={
                        "grant_type": "client_credentials",
                ...
                yield _normalize_text_response(response.get("result"), llm_provider)
...
async def generate_script_stream(
    ...
    logger.info(
        "generating video script (streaming): "
        f"subject={video_subject}, paragraph_number={paragraph_number}, "
        f"has_custom_prompt={bool(video_script_prompt.strip())}, "
        f"has_custom_system_prompt={bool(custom_system_prompt.strip())}"
    )
    ...
    for i in range(_max_retries):
        try:
            # 使用流式生成
            async for chunk in _generate_response_stream(prompt=prompt):
                if chunk:
                    # 检查是否包含错误信息
                    if "当日额度已消耗完" in chunk:
                        raise ValueError(chunk)
                    accumulated_script += chunk
                    # 实时返回清理后的文本块
                    cleaned_chunk = format_response(chunk)
                    if cleaned_chunk.strip():
                        yield cleaned_chunk
            
            # 流式生成完成后，检查最终结果
            if accumulated_script:
                final_script = format_response(accumulated_script)
                if "Error: " in final_script:
                    logger.error(f"failed to generate video script: {final_script}")
                else:
                    logger.success(f"completed: \n{final_script}")
    ...
    if not accumulated_script and i >= _max_retries - 1:
